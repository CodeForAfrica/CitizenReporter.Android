...
public class Storyboard extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
  ...
  @BindView(R.id.story_cause) EditText story_cause;
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (action.equals(Constants.ACTION_EDIT_VIEW_STORY)) {
      ...
      if (storyID > -1) {
        ...
        Toast.makeText(this, "This activeStory can not be found", Toast.LENGTH_SHORT).show();
    ...
    switch (requestCode) {
      ...
      case Constants.REQUEST_RECORD_AUDIO:
        if (resultCode == RESULT_OK) {
          ...
          Toast.makeText(this, "Audio recorded successfully! " + audio_path, Toast.LENGTH_SHORT)
              .show();
      ...
      case RequestCodes.OVERLAY_CAMERA:
        if (resultCode == Constants.CAMERA_MODE) {
          ...
          StoryBoardUtils.launchVideoCamera_SD(Storyboard.this,
              new StoryBoardUtils.LaunchVideoCameraCallback() {
                @Override public void onMediaCapturePathReady(String mediaCapturePath) {
                  //                            mMediaCapturePath = mediaCapturePath;
  ...
  @OnClick(R.id.storybaord_date) public void openCalendar() {
  ...
  public void startOverlayCamera(final Activity activity, Context context, final int mode) {
    ...
    SceneAdapter scenesAdapter =
        new SceneAdapter(activity, sceneTitles, sceneDescriptions, sceneImages, R.layout.row_scene);
    sceneslist.setAdapter(scenesAdapter);

    sceneslist.setOnItemClickListener(new AdapterView.OnItemClickListener() {
      @Override public void onItemClick(AdapterView<?> adapterView, View view, int j, long l) {
