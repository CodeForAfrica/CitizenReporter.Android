...
public class LocalDataHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_ASSIGNMENTS_TABLE = "CREATE TABLE "
                + Constants.ASSIGNMENTS_TABLE_NAME
                + "("
                + Constants.KEY_ID
                + " INTEGER PRIMARY KEY,"
                ...
                + " TEXT,"
                + Constants.KEY_ASSIGNMENT_UPDATED
                + " LONG,"
    ...
    public List<Assignment> getAssignments() {
        ...
        Cursor cursor = db.query(Constants.ASSIGNMENTS_TABLE_NAME,
                ...
                }, null, null, null, null, Constants.KEY_ASSIGNMENT_TITLE + " DESC");
    ...
    public long saveStory(Story story) {
        ...
        values.put(Constants.ITEM_INDEX, story.getItem_index());
    ...
    public Story getStory(long id) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(Constants.STORIES_TABLE_NAME, new String[]{
                Constants.KEY_ID, Constants.KEY_TITLE, Constants.KEY_WHY, Constants.KEY_WHO,
                Constants.KEY_AUTHOR, Constants.KEY_AUTHOR_ID, Constants.KEY_MEDIA, Constants.KEY_UPLOADED,
                Constants.KEY_WHERE, Constants.KEY_ASSIGNMENT_ID, Constants.KEY_UPDATED, Constants.KEY_WHEN,
                Constants.ITEM_INDEX
        ...
        story.setLocal_id(Integer.parseInt(cursor.getString(cursor.getColumnIndex(Constants.KEY_ID))));
        ...
        story.setItem_index(cursor.getString(cursor.getColumnIndex(Constants.ITEM_INDEX)));

        // convert timestamp to something readable
        java.text.DateFormat dateFormat = java.text.DateFormat.getDateInstance();
        String formattedDate =
                dateFormat.format(new Date(cursor.getLong(cursor.getColumnIndex(Constants.KEY_UPDATED))));
    ...
    public List<Story> getAllStories() {
        ...
        Cursor cursor = db.query(Constants.STORIES_TABLE_NAME, new String[]{
                ...
                Constants.KEY_WHERE, Constants.KEY_ASSIGNMENT_ID, Constants.KEY_UPDATED, Constants.KEY_WHEN,
                Constants.ITEM_INDEX
        }, null, null, null, null, Constants.KEY_TITLE + " DESC");
        if (cursor.moveToFirst()) {
            do {
                Story story = new Story();
                story.setLocal_id(
                ...
                String formattedDate = dateFormat.format(
                        new Date(cursor.getLong(cursor.getColumnIndex(Constants.KEY_UPDATED))));
                story.setUpdated(formattedDate);
    ...
    public int updateStory(Story story) {
        ...
        return db.update(Constants.STORIES_TABLE_NAME, values, Constants.KEY_ID + "=?",
                new String[]{String.valueOf(story.getLocal_id())});
    }

    public void deleteStory(String position) {
    SQLiteDatabase db = this.getWritableDatabase();
    db.delete(Constants.STORIES_TABLE_NAME, Constants.ITEM_INDEX + "=?",
        new String[] { position });
    db.close();
//        SQLiteDatabase db = getWritableDatabase();
//        db.execSQL("delete from " + Constants.STORIES_TABLE_NAME + " where " + Constants.ITEM_INDEX + " = " + position);
//        db.close();
