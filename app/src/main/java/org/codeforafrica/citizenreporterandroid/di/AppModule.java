...
@Module public class AppModule {
  ...
  @Provides @Named(NAME_BASE_URL) String provideBaseUrlString() {
    return Constants.BASE_URL;
  }

  @Provides @Singleton Converter.Factory provideGsonConverter() {
    return GsonConverterFactory.create();
  }

  @Provides @Singleton Retrofit provideRetrofit(Converter.Factory converter,
      @Named(NAME_BASE_URL) String baseUrl) {
    return new Retrofit.Builder().baseUrl(baseUrl).addConverterFactory(converter).build();
  }

  @Provides @Singleton CReporterAPI provideUsdaApi(Retrofit retrofit) {
    return retrofit.create(CReporterAPI.class);
  }

  @Provides @Named(NAME_PREF_FILE) String provideSharedPrefFileName() {
